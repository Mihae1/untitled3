...
public class oct15 {
    ...
    public static void main(String[] args) {
        System.out.println("Anzahl Character: " + text.length());
        System.out.println("Anzahl alphanumerische Character: " + countAlphaNumericChars(text));
        System.out.println("Anzahl Wörter: " + normalize(text).split(" ").length);
    }

    private static int countAlphaNumericChars(String textToCheck) {
        int counter = 0;
        for(char character : textToCheck.toCharArray()){
    ...
    public static String normalize(String textToNormalize) {
        textToNormalize = textToNormalize.replace("."," ");
        textToNormalize = textToNormalize.replace(","," ");
        textToNormalize = textToNormalize.replace(":"," ");
        textToNormalize = textToNormalize.replace("!"," ");
        textToNormalize = textToNormalize.replace("?"," ");
        ...
        textToNormalize = textToNormalize.replace("\""," ");
        textToNormalize = textToNormalize.replace("\n"," ");
        while(textToNormalize.contains("  ")){
            textToNormalize = textToNormalize.replace("  "," ");
        }
        return textToNormalize.toLowerCase();
    }
}
